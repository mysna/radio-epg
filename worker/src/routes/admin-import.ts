...
async function buildStatements(
  ...
  const broadcasters = new Set(batch.channels.map((channel) => channel.broadcaster_id));
  for (const broadcasterId of broadcasters) {
    statements.push(
      database
        ...
        .bind(broadcasterId, batch.source.name),
    );
  }
  ...
  for (const channel of batch.channels) {
    ...
    const aliases = [
      ...channel.radio_ids.map((value) => ({ type: "radio_id", value })),
      { type: "tuple", value: tupleAlias(channel) },
    ];
    for (const alias of aliases) {
      statements.push(
        database
          ...
          .bind(channel.channel_id, alias.type, alias.value),
      );
    }
  }
    ...
    statements.push(
      database
        ...
        .bind(
          ...
          JSON.stringify(program.hosts),
          program.genre ?? null,
          program.homepage_url ?? null,
        ),
    );
  }
  ...
  const scopes = new Set(
    batch.schedules.map(
      (event) => `${event.source_id}\u0000${event.channel_id}\u0000${event.broadcast_date}`,
    ),
  );
  for (const scope of scopes) {
    const [sourceId, channelId, broadcastDate] = scope.split("\u0000");
    statements.push(
      database
        .prepare(
          "DELETE FROM schedule_events WHERE source_id = ? AND channel_id = ? AND broadcast_date = ?",
        )
        .bind(sourceId, channelId, broadcastDate),
    );
  }
...
adminImport.post("/", async (context) => {
  ...
  try {
    const statements = await buildStatements(context.env.DB, parsed.data, payloadHash);
