...
def test_community_family_channels_were_removed_from_the_catalog() -> None:
...
def test_task_12_mapping_completes_all_catalog_identities() -> None:
    report = build_coverage(ROOT, require_accounted=True)

    assert report.accounted_count == report.catalog_count == 171
