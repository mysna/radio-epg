...
실패해도 정리 작업은 계속 실행하고, 원래 수집 실패 상태는 workflow에 그대로 남긴다.
편성은 요일 슬롯(일~토)으로 보관하며, 이 작업은 같은 요일 슬롯에 더 최근 방송일 편성이
있을 때 이전 편성만 삭제한다. 방송사가 편성을 올리지 않은 날("편성표 없음")은 수집
성공으로 기록하고 기존 요일 편성을 그대로 제공한다. 프로그램, 채널, 별칭은 보존한다.
반복 실행해도 결과가 달라지지 않는 멱등 유지보수 작업이다.
