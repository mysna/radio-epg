...
편성을 공개 API로 제공한다. 이미지는 출처 메타데이터와 함께 R2에 저장한다.
...
- `worker/`: Hono 기반 Worker, D1 마이그레이션, R2 이미지 API
...
명령은 저장소의 Wrangler 4 잠금 버전을 사용한다. Cloudflare 계정에서 Workers, D1, R2를
...
npx wrangler r2 bucket create radio-epg-images --storage-class Standard
```

`wrangler d1 create`가 출력한 UUID를 `worker/wrangler.toml`의 `database_id`에 복사한다.
R2 binding은 `IMAGES`, bucket 이름은 `radio-epg-images`를 유지한다. 이 서비스는 이미지를
자주 조회하고 30일 미만으로 교체할 수 있으므로 R2 Standard를 사용한다. R2 무료 구간은
Standard에만 적용되고 Infrequent Access는 조회 요금과 최소 보관 기간이 있다.

공식 참고 문서: [D1 Wrangler 명령](https://developers.cloudflare.com/d1/wrangler-commands/),
[R2 Wrangler 명령](https://developers.cloudflare.com/r2/reference/wrangler-commands/),
...
일일 수집 schedule은 UTC `17 19 * * *`, 즉 다음 날 **04:17 KST**다. GitHub 예약 실행은
...
작업은 종료 후 30일이 지난 편성 이벤트만 삭제하며 프로그램, 채널, 별칭, 이미지
메타데이터는 보존한다. 같은 시각에 반복 실행해도 결과가 달라지지 않는 멱등 유지보수
작업이다.
...
이미지는 원본 URL, 원본 페이지, author/license/attribution, 확인 시각과 content hash를
D1에 보관한다. API가 제공하는 이미지를 UI에 표시할 때 해당 attribution을 함께 노출한다.
권리가 불명확한 이미지는 업로드하지 않는다.

삭제 요청을 받으면 asset ID, 요청 근거와 UTC 시각을 기록한 뒤 인증 API를 호출한다.

```bash
curl --fail --request POST "$API/v1/admin/takedown" \
  --header "Authorization: Bearer <INGEST_TOKEN>" \
  --header 'Content-Type: application/json' \
  --data '{
    "asset_id": "<ASSET_SHA256>",
    "reason": "rights holder request <TICKET_ID>",
    "requested_at": "2026-07-13T12:00:00Z"
  }'
```

이 요청은 entity 연결을 해제하고 R2 variant를 지우며 hash와 source URL을 영구 blocklist에
남긴다. 같은 이미지의 재업로드는 `409 image_blocked`로 거절된다. 삭제 전에 ticket과
메타데이터를 감사 기록으로 보존하고 토큰이나 이미지 원본을 공개 issue에 첨부하지 않는다.

## 무료 사용량 모니터링

2026-07-13 기준 공식 무료 구간의 핵심 수치는 Workers 100,000 요청/일, D1 5백만 row
read/일·100,000 row write/일·계정 전체 5GB, R2 Standard 10GB-month·Class A 1백만
작업/월·Class B 1천만 작업/월이다. 수치는 바뀔 수 있으므로 운영 전
[Workers pricing](https://developers.cloudflare.com/workers/platform/pricing/),
[D1 pricing](https://developers.cloudflare.com/d1/platform/pricing/),
[R2 pricing](https://developers.cloudflare.com/r2/pricing/)을 다시 확인한다.
...
- R2 저장량, Class A/B operation, 이미지 404 비율
...
R2는 객체와 D1의 `image_variants.r2_key` 메타데이터가 함께 있어야 복구된다. S3 호환
도구로 bucket을 versioned backup 저장소에 복제하고, 샘플 객체의 hash·MIME·크기를 D1과
대조한다. 삭제 요청으로 blocklist된 객체는 백업에서 서비스 bucket으로 복원하지 않는다.
...
`Workers Scripts Edit`, `D1 Edit`, `Workers R2 Storage Edit`를 갖는지 확인한다. 적용된
...
### 이미지가 404이거나 MIME이 다름

D1의 asset/variant가 `available = 1`인지, R2 key가 존재하는지, object Content-Type이 D1의
`mime_type`과 같은지 확인한다. takedown된 hash는 다시 넣지 않는다. 메타데이터만 남거나
객체만 남은 경우 원본 출처와 권리를 재검증한 뒤 인증 image ingestion으로 복구한다.
